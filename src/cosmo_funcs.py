...
def Hubble(z, omega_matter):
    ...
    Flat LCDM Cosmology specified in global variables.
    In units of km/s/Mpc h^-1.
...
def inverse_hubble(z, omega_matter):
    """Get the inverse Hubble parameter at redshift z.
...
def comoving_distance(z, omega_matter):
    ...
    """
    C = 299792.458  # km/s
    integral_result = simpson_integrate_inv_hubble(
        inverse_hubble, 0.0, z, 1000, omega_matter
    )
...
def comoving_volume(z, omega_matter):
    ...
    return (4.0 / 3.0) * np.pi * d_c**3  # in Mpc^3 h^-3
...
def find_projected_separation(ra1, dec1, ra2, dec2, z1, omega_matter):
    ...
    return np.radians(angular_sep(ra1, dec1, ra2, dec2)) * comoving_distance(
        z1, omega_matter
    )  # in Mpc h^-1
...
def luminosity_to_mag(L, M_sun):
    ...
    return (
        -2.5 * np.log10(L * 10**14) + M_sun
    )  # Used to be no 2.5 and divided by 0.4 instead
