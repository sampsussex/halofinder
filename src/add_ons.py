...
def get_completeness(ra_targeted, dec_targeted, ra_observed, dec_observed, redshift_observed):
    nessie_cat = RedshiftCatalog(ra_observed, dec_observed, redshift_observed)
    ...
    completeness = nessie_cat.calculate_completeness(ra_targeted, dec_targeted, angles)
    return completeness
...
def reformat_sharks(in_filepath, out_filepath, peturbed_stellar_masses=False, region= 'wide', masked=False):
    ...
    mask = (sharks['mass_stellar_total'] > 8) & (sharks['mag_Z_VISTA'] > -99) & (sharks['redshift_observed'] < 0.2) #& (sharks['mag_Z_VISTA'] < 21.1) #& (sharks['dec'] > -3.95)
    if region == 'wide':
        mask = mask & (sharks['mag_Z_VISTA'] < 21.1)
    elif region == 'deep':
        mask = (
                mask & 
    ...
    sharks['DM'] = cosmo.distmod(sharks['redshift_cosmological']).value
    ...
    if mask == False:
        ...
        sharks_observed = sharks[sharks['observed'] == True].reset_index(drop=True)
