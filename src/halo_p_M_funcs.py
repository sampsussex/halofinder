...
def find_concentration_ratio(
    halo_mass, z=None, concentraion_relation="DuttonMaccio14", delta_crit=200.0
):
    ...
    if concentraion_relation == "DuttonMaccio14":
        ...
        raise ValueError(
            "Invalid concentration ratio method. Choose 'Maccio07' or 'Maccio08'."
        )
...
def f_x(scale_radius, projected_sep):
    ...
    if abs(x - 1.0) < 1e-12:  # Avoid numerical issues of sqrt near 0.
        ...
        return (1 - np.log((1 + np.sqrt(1 - x**2)) / x) / np.sqrt(1 - x**2)) / (
            x**2 - 1
        )
    elif x > 1:
...
@njit
def find_delta_bar(halo_concentration, delta_crit=200.0):
    ...
    return (
        delta_crit
        * halo_concentration**3
        / (
            3 * np.log(1 + halo_concentration)
            - halo_concentration / (1 + halo_concentration)
        )
    )
...
@njit
def find_sigma_sqr(halo_mass, z_group, omega_matter, delta_crit=200.0):
...
def find_p_delta_z(delta_z, z_group, halo_mass, omega_matter):
    ...
    return (
        C
        / (np.sqrt(2 * np.pi) * np.sqrt(sigma_sqr))
        * np.exp(-((C * delta_z) ** 2) / (2 * sigma_sqr))
    )
